...
<div class="row">
    ...
    <div class="col-sm-4" style="border-left: 4px solid #13428f; margin-left: 20px; align-self: center;">
           <p style="margin-left:20px;color:#13428f"><big><i>
            Language models are capable of extracting factual knowledge from the diverse texts contained within their training corpora (such as history books or cooking recipes)—for instance, understanding who Napoleon Bonaparte is. They can also deduce knowledge about relationships between objects (a knife cuts vegetables, but a spoon does not). As such, they seem capable of capturing a limited model of our world by predicting the outcomes of common and well-documented interactions found in their training corpus (e.g., if you blow on a candle, it goes out).

            This mode of learning is also used by humans, particularly by children. Stories, for example, appear to play an essential role in learning about the world and understanding the meaning behind certain actions [1]. However, this passive learning is not the only method humans rely on. Language acquisition itself involves interactions and occurs within the constraints of a sociocultural environment where the use of words is tied to physical concepts or our social framework. The fact that our words are grounded—linked to the world around us—is a fundamental aspect of the language we use to communicate, convey, and imagine. The way language models are currently trained, however, does not incorporate such grounding.
          </i></big></p>
    </div>
</div>
</div>
